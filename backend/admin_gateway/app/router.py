...
async def Central_server_health(request: Request) -> JSONResponse:
	"""Return central service health: proxy to /health or use recorded status.
	...
	try:
		...
		logger.info("Central service sehat hai (status=%s)", getattr(resp, "status_code", "unknown"))
		return resp
	except HTTPException as exc:
		logger.error("Central service health check fail hua — gateway se connect nahi ho paaya: %s", getattr(exc, "detail", str(exc)))
		status = SERVICE_STATUS.get(service, {"healthy": False, "error": getattr(exc, "detail", str(exc))})
		logger.info("Central service ka recorded status: %s", status)
